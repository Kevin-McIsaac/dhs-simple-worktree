 ...
 *
 * One job: cut a git worktree under `<repo>/.wt/<slug>` and hand back its cwd —
 * the CLIENT then births the session in the tree through DSH's own
 * `sessions.create({ cwd })`, which is the one moment a session's cwd is
 * settable. This half never creates sessions and never registers the tree as a
 * workspace: `attachSession` refuses any session whose cwd differs from its
 * workspace path, so a worktree session can never be a registry member of the
 * project — the sidebar patch re-homes it under the project row instead.
 *
 * Routes (registered on the host webServer, same idiom as dsh-git-badge):
 *   POST /api/worktree-session/create   body { workspaceId } or { sessionId }
 *       fetch origin, cut `git worktree add -b wt/<slug> .wt/<slug> <base>`,
 *       symlink the checkout's .env into the tree, run an optional
 *       `.worktree-bootstrap` script inside the tree, return
 *       { cwd, branch, base, slug, envLinked, bootstrap, warnings }.
 *   GET  /api/worktree-session/list?session=…
 *       the repo's .wt trees with their safety facts (dirty, unmerged) for the
 *       client's remove popover.
 *   POST /api/worktree-session/cleanup  body { path, force? }
 *       remove tree + branch (+ any v0.1-era workspace registration for the
 *       path). Refuses anything the git-worktree skill calls dangerous unless
 *       force is set: a dirty tree, or a branch with commits no remote contains
 *       and no merged PR.
 *
 * Safety invariants:
...
/**
 * Progress plumbing: one SSE stream per project workspace id. The client
 * subscribes BEFORE clicking create, so every step lands live. A short event
 * log per key lets a late subscriber catch up. Same event-driven idiom as
 * dsh-git-badge — no polling anywhere.
 */
const progressStreams = new Map();
const progressLog = new Map();

export function emitProgress(key, step, extra = {}) {
	const event = { at: Date.now(), step, ...extra };
	const log = [...(progressLog.get(key) ?? []), event].slice(-50);
	progressLog.set(key, log);
	for (const res of progressStreams.get(key) ?? []) {
		try {
			res.write(`data: ${JSON.stringify(event)}\n\n`);
		} catch {
			// a dead stream is cleaned up by its close handler
		}
	}
}

/**
 * Cut the worktree for one session's repo. The heavy optional step — the
 * repo's `.worktree-bootstrap` hook — runs in the BACKGROUND after this
 * returns: the session must not wait on a hook that may legitimately take
 * minutes. Steps stream to `progressKey` (the project workspace id the client
 * clicked); only fatal steps (repo/git/base/slug/worktree) throw.
 */
export async function createWorktree(workspaceRegistry, repoPath, progressKey, now = new Date()) {
	...
	const warnings = [];
	// No pre-cut fetch: the branch is cut from last-known refs (sub-second, the
	// git-worktree skill's own flow). The git badge keeps refs fresh instead.

	const base = await baseRef(repo);
	const slug = await uniqueSlug(repo, now);
	const branch = `wt/${slug}`;
	emitProgress(progressKey, "cutting", { branch });
	...
	// Register the tree as a workspace — the session MUST be a registry member:
	// the conversation hero disables the composer ("Choose a workspace to
	// start") for sessions with no workspace, and attachSession pins cwd to the
	// workspace path, so membership is only possible with the tree registered.
	// The sidebar patch absorbs the worktree's group into its project's row, so
	// registration never shows up as a separate project.
	const workspace = await workspaceRegistry.create(treePath);
	emitProgress(progressKey, "ready", { workspaceId: workspace.id, cwd: treePath, branch });

	// The repo's own bootstrap hook, if it declares one: an executable
	// `.worktree-bootstrap` at the repo root, run INSIDE the tree — in the
	// background, so the session opens without waiting on it. Its outcome
	// streams to the progress feed and never fails the session.
	...
	if (existsSync(bootstrapPath)) {
		...
		if (!executable) {
			...
			emitProgress(progressKey, "bootstrap", { state: "failed", reason: bootstrap.reason });
		} else {
			bootstrap = { ran: true, async: true };
			emitProgress(progressKey, "bootstrap", { state: "started" });
			execFile(bootstrapPath, [], { cwd: treePath, timeout: config.bootstrapTimeoutMs, windowsHide: true }, (error, _stdout, stderr) => {
				const outcome = error === void 0 || error === null
					? { state: "ok" }
					: error.killed === true
						? { state: "timeout" }
						: { state: "failed", reason: `exit ${error.code}`, stderr: String(stderr).slice(-2000) };
				emitProgress(progressKey, "bootstrap", outcome);
			});
		}
	}

	return { workspaceId: workspace.id, cwd: treePath, branch, base, slug, envLinked, bootstrap, warnings };
}
...
/**
 * Remove one worktree tree + its wt/ branch (+ any workspace registration a
 * v0.1 install left behind for it). Refuses (409-shaped result) unless force:
 *   - the tree is dirty;
 ...
 */
export async function cleanupWorktree(workspaceRegistry, treePath, force = false) {
	...
	// v0.1 installs registered worktrees as workspaces; if one still exists for
	// this path, retire it. Absence is the normal case and is not a warning.
	const entity = await workspaceRegistry.resolveByPath(tree).catch(() => void 0);
	if (entity !== void 0 && entity !== null) {
		try {
			await workspaceRegistry.delete({ workspaceId: entity.id });
		} catch (error) {
			warnings.push(`stale workspace "${entity.id}" could not be unregistered: ${String(error)}`);
		}
	}
...
function apply(ctx) {
		...
		handler: async (req, res) => {
			try {
				...
				if (typeof body.workspaceId === "string" && body.workspaceId !== "") {
					...
					const entity = ctx.workspaceRegistry.get(body.workspaceId);
					if (entity === void 0) return sendJson(res, 400, { error: `workspace "${body.workspaceId}" not found` });
					repoPath = entity.path;
					progressKey = entity.id;
				} else if (typeof body.sessionId === "string" && body.sessionId !== "") {
					// The chip flow resolves the repo from the conversation's own cwd.
					const workspace = await resolveSessionWorkspace(ctx.workspaceRegistry, deps.resolveSessionCwd, body.sessionId);
					repoPath = workspace.path;
					progressKey = workspace.id;
				} else {
					return sendJson(res, 400, { error: "body must be { workspaceId } or { sessionId }" });
				}
				const result = await createWorktree(ctx.workspaceRegistry, repoPath, progressKey);
	...
	ctx.effect(() => ctx.webServer.register({
		kind: "exact",
		path: "/api/worktree-session/progress",
		handler: async (req, res) => {
			try {
				const url = new URL(req.url, "http://localhost");
				const key = url.searchParams.get("workspaceId");
				if (key === null || key === "") return sendJson(res, 400, { error: "query must be ?workspaceId=<project workspace id>" });
				res.writeHead(200, { "content-type": "text/event-stream", "cache-control": "no-cache" });
				// replay the recent log so a late subscriber catches up, then stream live
				for (const event of progressLog.get(key) ?? []) res.write(`data: ${JSON.stringify(event)}\n\n`);
				let streams = progressStreams.get(key);
				if (streams === void 0) {
					streams = new Set();
					progressStreams.set(key, streams);
				}
				streams.add(res);
				req.on("close", () => {
					streams.delete(res);
	...
	}), "worktree-session: progress route");
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			try {
				const body = await readJsonBody(req);
				if (body === void 0) return sendJson(res, 400, { error: "request body is not valid JSON" });
				if (typeof body.path !== "string" || body.path === "") return sendJson(res, 400, { error: "body must be { path, force? }" });
				const result = await cleanupWorktree(ctx.workspaceRegistry, body.path, body.force === true);
...
/** Test-only export: the buffered progress log for one key (badge idiom). */
export function progressEventsFor(key) {
	return [...(progressLog.get(key) ?? [])];
}
