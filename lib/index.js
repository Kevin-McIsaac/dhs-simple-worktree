 ...
 */
export async function createWorktree(repoPath, now = new Date()) {
	...
	// Trees are deliberately NOT registered as workspaces: attachSession refuses
	// any session whose cwd differs from its workspace path, so a worktree
	// session can never be a registry member of the project. The client births
	// the session with a raw cwd; the sidebar patch re-homes it under the row.
	return { cwd: treePath, branch, base, slug, envLinked, bootstrap, warnings };
}
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			try {
				...
				const result = await createWorktree(repoPath);
