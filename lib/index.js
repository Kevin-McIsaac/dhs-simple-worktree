	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			try {
				...
				if (typeof body.workspaceId !== "string" || body.workspaceId === "") return sendJson(res, 400, { error: "body must be { workspaceId }" });
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			try {
				...
				if (workspaceId === null || workspaceId === "") return sendJson(res, 400, { error: "query must be ?workspaceId=<project workspace id>" });
				const entity = ctx.workspaceRegistry.get(workspaceId);
				if (entity === void 0) return sendJson(res, 400, { error: `workspace "${workspaceId}" not found` });
