 ...
 *   POST /api/worktree-session/create   body { workspaceId }
 ...
 *   GET  /api/worktree-session/list?workspaceId=…
...
/** Run one command; resolve { ok, stdout, stderr } — failures are data, not throws. */
function run(command, cwd, args, timeoutMs = config.gitTimeoutMs) {
...
function runGit(cwd, args, timeoutMs = config.gitTimeoutMs) {
	return run("git", cwd, args, timeoutMs);
}

/** [repoPath, rest] when p sits under a `/.wt/` directory, else void 0. */
function splitWtPath(p) {
	const i = p.indexOf("/.wt/");
	return i === -1 ? void 0 : [p.slice(0, i), p.slice(i + "/.wt/".length)];
}
...
export async function baseRef(repoPath) {
	...
	if (head.ok) {
		const verify = await runGit(repoPath, ["rev-parse", "--verify", "--quiet", head.stdout.trim()]);
		if (verify.ok) return head.stdout.trim();
	}
...
export async function branchMerged(repoPath, branch) {
	...
	const gh = await run("gh", repoPath, ["pr", "view", branch.replace(/^wt\//, ""), "--json", "state"]);
 ...
 * minutes. Only fatal steps (repo/git/base/slug/worktree) throw.
 */
export async function createWorktree(workspaceRegistry, repoPath, now = new Date()) {
	...
	let workspace;
	try {
		workspace = await workspaceRegistry.create(treePath);
	} catch (error) {
		// The tree is cut but can never become a session — remove it and its
		// branch so a failed create leaves nothing behind (best effort).
		await runGit(repo, ["worktree", "remove", "--force", treePath]);
	...
	// background, so the session opens without waiting on it. A failure is
	// logged, never fails the session.
	const bootstrapPath = join(repo, ".worktree-bootstrap");
	let bootstrap = { ran: false };
	if (existsSync(bootstrapPath)) {
		let executable = false;
		try {
			const st = statSync(bootstrapPath);
		...
		if (!executable) {
			...
			execFile(bootstrapPath, [], { cwd: treePath, timeout: config.bootstrapTimeoutMs, windowsHide: true }, (error, _stdout, stderr) => {
				if (error !== void 0 && error !== null) {
					console.warn("[dsh-worktree-session] .worktree-bootstrap failed:", error.killed === true ? "timed out" : `exit ${error.code}`, String(stderr).slice(-2000));
				}
			});
		}
	}
 ...
 * Refuses (409-shaped result) unless force:
 *   - the tree is dirty;
...
export async function cleanupWorktree(workspaceRegistry, treePath, force = false) {
	...
	const wtSplit = splitWtPath(tree);
	if (wtSplit === void 0) throw new Error(`refusing to remove "${tree}": it is not under a .wt/ directory`);
...
export async function listWorktrees(workspaceRegistry, repoPath) {
	...
	for (const treePath of paths) {
		const wtSplit = splitWtPath(treePath);
		if (wtSplit === void 0) continue;
...
/** Wrap a route handler: thrown errors become a 400 JSON response. */
const route = (handler) => async (req, res) => {
	try { await handler(req, res); } catch (error) { sendJson(res, 400, { error: String(error?.message ?? error) }); }
};
/** 400 with a JSON error body. */
const badRequest = (res, error) => sendJson(res, 400, { error });

/** Host plugin body — register the two routes (create, cleanup). */
function apply(ctx) {
	ctx.effect(() => ctx.webServer.register({
		...
		handler: route(async (req, res) => {
			...
			if (body === void 0) return badRequest(res, "request body is not valid JSON");
			if (typeof body.workspaceId !== "string" || body.workspaceId === "") return badRequest(res, "body must be { workspaceId }");
			const entity = ctx.workspaceRegistry.get(body.workspaceId);
			if (entity === void 0) return badRequest(res, `workspace "${body.workspaceId}" not found`);
			const result = await createWorktree(ctx.workspaceRegistry, entity.path);
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: route(async (req, res) => {
			...
			if (workspaceId === null || workspaceId === "") return badRequest(res, "query must be ?workspaceId=<project workspace id>");
			const entity = ctx.workspaceRegistry.get(workspaceId);
			if (entity === void 0) return badRequest(res, `workspace "${workspaceId}" not found`);
			sendJson(res, 200, { worktrees: await listWorktrees(ctx.workspaceRegistry, entity.path) });
		}),
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: route(async (req, res) => {
			const body = await readJsonBody(req);
			if (body === void 0) return badRequest(res, "request body is not valid JSON");
			if (typeof body.path !== "string" || body.path === "") return badRequest(res, "body must be { path, force? }");
