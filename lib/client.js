	...
	factory: (require) => {
		...
		function makeChip(ctx) {
			...
			function WorktreeChip({ sessionId }) {
				...
				// Membership REQUIRES the tree registered (the hero disables the
				// composer otherwise); the sidebar patch absorbs the worktree's
				// group into the project row, so registration never shows as a
				// separate project.
				const create = async () => {
					if (busy) return;
					setBusy(true);
